...
export const configValidationSchema = Joi.object({
  ...
  CLIENT_URL: Joi.string().uri().default('http://localhost:5173').messages({
  ...
  DATABASE_URL: Joi.string()
    ...
    .messages({
      'any.required':
        'DATABASE_URL is required — get from Supabase → Settings → Database → URI',
    }),
  ...
  SUPABASE_URL: Joi.string()
    ...
    .messages({
      'any.required':
        'SUPABASE_URL is required — get from Supabase → Settings → API',
    }),
  ...
  SUPABASE_SERVICE_KEY: Joi.string()
    ...
    .messages({
      'any.required':
        'SUPABASE_SERVICE_KEY is required — use service_role key NOT anon key',
    }),
  ...
  REDIS_URL: Joi.string()
    .required()
    .messages({
      'any.required': 'REDIS_URL is required — get from upstash.com',
    }),
